...
namespace Geometry
{
    internal class IntersectionOperator : IOperator
    {
        public OperatorType Type => OperatorType.Intersection;
        public IReadOnlyCollection<Vector2> BasicPoints => throw new NotImplementedException();
        public string Name => "IntersectionOperator";
        ...
        public bool IsPointInFigure(Vector2 position, double eps)
        {
            foreach (IGeometry operand in operands)
                if (!operand.IsPointInFigure(position, eps))
                    return false;
            return true;
        }
